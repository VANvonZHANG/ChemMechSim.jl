...
import ChemMechSim: catalyst_lowering, direct_mtk_lowering, RateCtx
...
@testset "§3.4 #3: catalyst + direct rates share @species, unify into one ODESystem" begin
    ...
    rate_cat = catalyst_lowering(rxA, mech, cvar, nothing, 1, RateCtx(mech, cvar, nothing, 1, 1.0, nothing, nothing, Dict{Int,Any}(), nothing, Any[]))    # k_1_A * A    (Catalyst path)
    rate_dir = direct_mtk_lowering(rxB, mech, cvar, nothing, 2, RateCtx(mech, cvar, nothing, 2, 2.0, nothing, nothing, Dict{Int,Any}(), nothing, Any[]))  # k_2_A * A*M  (direct path)
...
@testset "§3.4 #2: T-dependent catalyst rate equals the direct path" begin
    ...
    rc = catalyst_lowering(rxn, mech, cvar, T, 1, RateCtx(mech, cvar, T, 1, 1.0, nothing, nothing, Dict{Int,Any}(), nothing, Any[]))
    rd = direct_mtk_lowering(rxn, mech, cvar, T, 1, RateCtx(mech, cvar, T, 1, 1.0, nothing, nothing, Dict{Int,Any}(), nothing, Any[]))
    @test isequal(rc, rd)
