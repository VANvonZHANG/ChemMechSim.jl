...
t_solve = @elapsed sol = simulate(
    ...
    # T is a parameter here (isothermal), so FBDF gets no analytic Jacobian — it forms one by
    # finite differences. Set jac=true to use the reaction-sharded analytic Jacobian instead;
    # that is a large separate build and is not needed for this demonstration.
